 ...
 * entryoff): a dated observation (checked once, under the exit-code
 * numbering in effect at the time, which reported this as exit 2 --
 * EX_REFUSED is 1 today; cmd_grow's own "refuse rather than guess" comment,
 * unchanged since, is why this is expected to still hold, not a re-check),
 * file left byte-for-byte unmodified on disk either way. So these three
