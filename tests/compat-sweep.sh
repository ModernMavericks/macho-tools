...
# ---- report -------------------------------------------------------------
{
    ...
    echo "#   declassify exits 1 (EX_REFUSED, as this build numbers it today -- a"
    echo "#   FRESH run of this generator reflects whatever cli/macho9.c currently"
    echo "#   says, unlike the checked-in tests/compat-matrix.tsv, which is a frozen"
    echo "#   measurement against the numbering in effect when IT was generated and"
    echo "#   is annotated separately rather than kept in sync with this text -- see"
    echo "#   its own header). That is deliberate -- cli/macho9.c's cmd_declassify"
    echo "#   names it as one of FOUR DELIBERATE DIVERGENCES FROM patch_macho -- but"
    echo "#   it is invisible to anyone grepping by class."
    echo "#"
