...
# ---- report -------------------------------------------------------------
{
    ...
    echo "#   declassify exits 2 (EX_REFUSED, AS NUMBERED WHEN THIS WAS MEASURED --"
    echo "#   see \"AGAINST WHAT\" above; a later commit renumbered EX_REFUSED to 1 and"
    echo "#   introduced EX_FAIL=2, so new_rc=2 in those two rows means the SAME"
    echo "#   refusal a live run of today's macho9 would report as 1, not 2. This is"
    echo "#   exactly the kind of thing \"CANNOT be regenerated\" means: the numbers"
    echo "#   below are frozen at what the numbering was on 2026-09-10, not updated"
    echo "#   when the numbering later changed). That divergence itself was deliberate"
    echo "#   even then -- cli/macho9.c's cmd_declassify names it as one of FOUR"
    echo "#   DELIBERATE DIVERGENCES FROM patch_macho -- but it is invisible to anyone"
    echo "#   grepping by class."
    echo "#"
