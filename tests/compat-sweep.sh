...
# ---- report -------------------------------------------------------------
{
    ...
    echo "#   were. Both are still deliberate -- cli/macho9.c's cmd_declassify names"
    echo "#   the EXIT CODES distinction as one of FOUR DELIBERATE DIVERGENCES FROM"
    echo "#   patch_macho -- but only the absent-file one is visible to anyone"
    echo "#   grepping by class today."
    echo "#"
