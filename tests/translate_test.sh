...
# invocation is worth more than one machotool command -- the whole
# `machotool edit FILE OUT -` here-document, terminator included.
#
...
ok cd-change    'machotool dylib f f.new -replace OLD NEW
mv -f f.new f'        -- change_dylib f -change OLD NEW
ok cd-delete    'machotool dylib f f.new -delete P
mv -f f.new f'               -- change_dylib f -delete P
ok cd-reexport  'machotool dylib f f.new -reexport P
mv -f f.new f'             -- change_dylib f -reexport P
ok cd-add       'machotool dylib f f.new -append P
mv -f f.new f'               -- change_dylib f -add P
ok cd-insert    'machotool dylib f f.new -insert P
mv -f f.new f'               -- change_dylib f -insert P
ok cd-rchange   'machotool rpath f f.new -replace OLD NEW
mv -f f.new f'        -- change_dylib f -change-rpath OLD NEW
ok cd-rdelete   'machotool rpath f f.new -delete P
mv -f f.new f'               -- change_dylib f -delete-rpath P
ok cd-radd      'machotool rpath f f.new -append P
mv -f f.new f'               -- change_dylib f -add-rpath P
ok cd-strip     'machotool lc f f.new -delete uuid
mv -f f.new f'               -- change_dylib f -strip-lc uuid
ok cd-grow      'machotool dylib f f.new --allow-grow -append P
...
# to the other.
ok cd-add-vs-insert 'machotool dylib f f.new -append A -insert B
...
ok cd-kind-uuid     'machotool lc f f.new -delete uuid
mv -f f.new f'           -- change_dylib f -strip-lc uuid
ok cd-kind-codesig  'machotool lc f f.new -delete codesig
mv -f f.new f'        -- change_dylib f -strip-lc codesig
ok cd-kind-srcver   'machotool lc f f.new -delete source-version
mv -f f.new f' -- change_dylib f -strip-lc source-version
ok cd-kind-buildver 'machotool lc f f.new -delete build-version
mv -f f.new f'  -- change_dylib f -strip-lc build-version
ok cd-kind-drs      'machotool lc f f.new -delete code-sign-drs
...
ok cd-repeat 'machotool dylib f f.new -replace A B -replace C D
mv -f f.new f' -- change_dylib f -change A B -change C D
ok cd-strip-repeat 'machotool lc f f.new -delete uuid -delete codesig
mv -f f.new f' -- change_dylib f -strip-lc uuid -strip-lc codesig

# ---- change_dylib: mixing families becomes ONE edit script ---------------
#
# One family is one verb, whose flags machotool batches. More than one family has
# no single verb to be, so it is one `machotool edit FILE OUT -` with the
...
ok cd-mixed-2 "machotool edit f f.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
dylib replace A B
MACHOTOOL_EDIT
mv -f f.new f" -- change_dylib f -strip-lc uuid -change A B

ok cd-mixed-3 "machotool edit f f.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
dylib append D
rpath append R
MACHOTOOL_EDIT
...
ok cd-mixed-order "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok cd-grow-mixed "machotool edit f f.new - <<'MACHOTOOL_EDIT'
allow-grow
load-command delete uuid
dylib replace A B
rpath append R
...
ok cd-insert-reverse "machotool edit f f.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
dylib insert B
dylib insert A
MACHOTOOL_EDIT
...
ok cd-production "machotool edit /tmp/c /tmp/c.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
load-command delete codesig
...
ok cd-characterize "machotool edit out out.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
load-command delete codesig
dylib replace /usr/lib/libSystem.B.dylib @loader_path/../S.dylib
MACHOTOOL_EDIT
...
# ---- fix_macho ----------------------------------------------------------
ok fm-change   'machotool dylib f f.new -replace OLD NEW
mv -f f.new f'         -- fix_macho f -change OLD NEW
ok fm-stripbv  'machotool lc f f.new -delete build-version
...
ok fm-rename   'machotool segment f f.new __DATA __D2
mv -f f.new f'            -- fix_macho f -rename_seg __DATA __D2
# `machotool segment` takes ONE pair, so two renames are two commands on their
...
ok fm-rename-2 "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok fm-all "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok fm-stripbv-twice 'machotool lc f f.new -delete build-version
mv -f f.new f' \
...
ok fm-chain "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok fm-chain-3 "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok fm-chain-empty "machotool edit f f.new - <<'MACHOTOOL_EDIT'
...
ok avm     'machotool minos f f.new 10.9
...
if [ "$mt_out_got" = 'machotool minos f /tmp/t.tmp 10.9' ]; then
    pass=$((pass + 1))
else
...
ok pm      'machotool declassify in out'        -- patch_macho in out
# ... EXCEPT when IN and OUT are the same file, which patch_macho allowed and
# `machotool declassify` now refuses. The teaching form has to be a pasteable
...
ok pm-same 'machotool declassify f f.new
...
if [ "$mt_out_got" = 'machotool declassify in /tmp/t.tmp' ]; then
    pass=$((pass + 1))
else
    printf 'FAIL pm-mt-out: got %s\n' "$mt_out_got" >&2; fail=$((fail + 1))
fi
mt_out_got=$( MT_OUT=/tmp/t.tmp /bin/sh "$TR" patch_macho f f )
if [ "$mt_out_got" = 'machotool declassify f /tmp/t.tmp' ]; then
    pass=$((pass + 1))
else
    printf 'FAIL pm-mt-out-same: got %s\n' "$mt_out_got" >&2; fail=$((fail + 1))
fi
ok rs      'machotool segment f f.new __DATA __DATA2
mv -f f.new f' -- rename_segment f __DATA __DATA2
ok rs-16   'machotool segment f f.new __DATA 1234567890123456
mv -f f.new f' -- rename_segment f __DATA 1234567890123456
# retag_swift_classes is variadic over FILES; machotool retag-swift takes one, so
...
ok rsc-1 'machotool retag-swift a a.new
mv -f a.new a'                                                    -- retag_swift_classes a
ok rsc-3 'machotool retag-swift a a.new
mv -f a.new a
machotool retag-swift b b.new
mv -f b.new b
machotool retag-swift c c.new
...
if [ "$rsc_out_got" = 'machotool retag-swift a /tmp/t.tmp' ]; then
    pass=$((pass + 1))
else
...
ok q-space "machotool segment 'a b' 'a b.new' __DATA __D2
mv -f 'a b.new' 'a b'"      -- rename_segment 'a b' __DATA __D2
ok q-quote "machotool declassify 'it'\\''s' out"      -- patch_macho "it's" out
ok q-empty "machotool segment f f.new __DATA ''
...
set --; eval "set -- $(printf '%s' "$line" | sed 's/^machotool //')"
...
if [ "$got" = '/opt/bin/machotool minos f f.new 10.9
mv -f f.new f' ]; then
    pass=$((pass + 1))
else
...
ok cd-chain-one-family 'machotool dylib f f.new -replace a b -replace b c
mv -f f.new f' \
    -- change_dylib f -change a b -change b c
ok fm-chain-one-family 'machotool dylib f f.new -replace a b -replace b c
mv -f f.new f' \
...
ok cd-self-replace "machotool edit f f.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
dylib replace a a
dylib replace c d
MACHOTOOL_EDIT
...
ok cap-strip-16-fits "machotool lc f f.new$(mkcap '-delete uuid' 16)
...
# counterpart at all -- machotool sees one rename at a time either way, its
...
ok fm-cap-change-32-fits "machotool dylib f f.new$(mkcap '-replace A B' 32)
mv -f f.new f" \
...
if [ -x /bin/ksh ]; then
    ...
    want="machotool edit f f.new - <<'MACHOTOOL_EDIT'
load-command delete uuid
dylib replace A B
rpath append R
MACHOTOOL_EDIT
