...
| `edit_test` | hermetic: `src/edit.c`'s execution model against a synthetic image written to a temp directory — statements apply in order and each sees the last one's result, the result lands in `OUT` with the input's mode, a refusal part-way leaves the input byte-for-byte and inode-for-inode untouched with no `OUT` or temp file created, an `OUT` that is the input is refused before anything is read, the final verify ignores `MACHO_NO_VERIFY`, and only a thin 64-bit Mach-O is accepted |
...
`machotool` uses three exit codes throughout. `verify`, `info`, `grow`, `minos`
...
- **Modern macOS SIGKILLs a resized binary** (code-signing enforcement: the
  ...
  yes) before it ever runs a machotool-modified binary; the grow/lc "still
