#!/bin/sh
# change_dylib -- a /bin/sh wrapper around machotool's `lc`, `dylib` and `rpath`.
#
...
# (src/rewrite.h). machotool's `dylib`, `rpath` and `lc` verbs parse their own
...
#
#     -strip-lc KIND        machotool lc    FILE OUT -delete KIND
#     -change O N           machotool dylib FILE OUT -replace O N
#     -delete P                          ... -delete P
#     -reexport P                        ... -reexport P
#     -add P                             ... -append P
#     -insert P                          ... -insert P
#     -change-rpath O N     machotool rpath FILE OUT -replace O N
...
# `machotool edit FILE OUT -` with the operations as statements on stdin, ordered
...
#
# THE CAPACITY CAPS ARE ENFORCED IN THE TRANSLATION, not by machotool. Both cap
# sites in cli/machotool.c carry a comment saying so and addressing whoever wrote
# this wrapper: machotool caps at the same numbers (MR_MAX_OPS=32, MR_MAX_STRIP=16)
...
# wrapper runs the emitted `machotool dylib`/`rpath`/`lc`/`edit` command and
...
# `machotool edit` speaks the same vocabulary for the same reasons: me_run
...
# verbs; what those rows now run is one `machotool edit`, so the counts still say
...
#       lines of machotool's are reshaped to get there, and both are consequences
...
#   MORE THAN ONE FAMILY -- 669 rows -- one `machotool edit`, and stdout DIFFERS,
...
#       because that is the path machotool was handed. Reproducing the C tool's
#       exact transcript would mean suppressing machotool's output and inventing
...
#       is empty by design (there is no machotool command that means "do
...
# sequence refused having already written. One `machotool edit` closes that at
...
# ---- THE IN-PLACE EDIT ---------------------------------------------------
#
# change_dylib rewrote FILE; `machotool dylib`/`rpath`/`lc` do not write the file
# they are given. So this wrapper does what the old tool looked like it did,
# safely, through the shared sequence machotool-compat.sh's "the install path"
...
# that case. `machotool edit`, the multi-family path, takes that temp as its OUT
...
#     having changed nothing. No machotool command reproduces that any more: a
#     verb that writes an output opens FILE O_RDONLY and has no opinion about
#     whether FILE is writable, and `machotool edit` installs by rename, which
...
#     which is the trade every wrapper on this path makes; machotool-compat.sh's
...
# patched, now fails -- `mkstemp: Permission denied`, from machotool's own write
...
# and there is no temp for machotool to write or for mw_finish to install. The C
